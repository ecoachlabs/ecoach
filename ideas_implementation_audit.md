...
| idea4.txt | read_complete | implemented_gap, partial | Added Beat Yesterday daily target generation, summary scoring, recovery-aware mode selection, climb trend projection, stronger outcome-aware replan/free-now scheduling that now reacts to repair success, failure, and return-loop pressure, and verified app-facing command/Tauri access for free-now and day-replan outputs; full teacher/risk layer still missing |
| idea5.txt | read_complete | implemented_gap, partial | Added elite session scoring, debrief recommendations, rolling profile updates, topic domination tracking, trap-sensitive elite blueprint biasing for low-domination/high-confusion topics, richer elite blueprint reports/profile reads, and verified app-facing command/Tauri access for elite profile, domination, blueprint, and blueprint-report outputs; session generation and richer authoring still missing |
| idea6.txt | read_complete | implemented_gap, partial | Added skill-level knowledge gap substrate plus richer repair-plan detail, actionable repair item guidance, candidate question routing, misconception/resource hints, duplicate-plan prevention, active-item eventing, solidification-session seeding, and stronger repair close/continue behavior driven by session outcomes; broader gap-engine orchestration still remains |
| idea7.txt | read_complete | implemented_gap, partial | Memory mode audit complete; live memory updates, due recheck listing, overdue decay scanning, outcome-aware return-loop planning, and topic-truth recompute from memory state now exist, but deeper long-horizon orchestration still remains |
| idea8.txt | read_complete | implemented_gap, partial | Same backend implications as idea7; live memory updates, due recheck listing, overdue decay scanning, outcome-aware return-loop planning, and topic-truth recompute from memory state now exist, but deeper long-horizon orchestration still remains |
...
| idea10.txt | read_complete | implemented_gap, partial | Added real MindStack and Tug-of-War runtime state reads, command-boundary coverage, app-facing Tauri registration on top of the game session backend, richer game summaries, trap session snapshots, clue reveal flow, and stronger trap review/remediation detail at the app boundary; broader game expansion still remains |
...
| idea13.txt | read_complete | implemented_gap, partial | Added past-paper question linking, family recurrence/coappearance analytics recompute, high-frequency family read models, inverse-pressure analytics, comeback-candidate family ranking based on historical strength plus dormancy, and app-facing command/Tauri access for comeback families, session remediation plans, and session evidence fabrics; richer paper intelligence still remains |
...
| idea15.txt | read_complete | implemented_gap, partial | Added live memory updates, due recheck listing, overdue decay scanning, memory return-loop session planning, topic-truth recompute from memory decay, and repair-outcome-aware return-loop escalation/downshifting; deeper adaptation across longer learning arcs still remains |
| idea16.txt | read_complete | implemented_gap, partial | Added generated library shelves, saved-question intelligence, weak-topic revision packs, topic relationship hints, teach-action plans, personalized learning paths, stronger continue-learning fallback, glossary bundle sequencing, deeper relationship traversal derived from learner state plus content graph links, and verified app-facing command/Tauri access for personalized learning paths and glossary bundle sequencing; richer personalization still remains |
| idea17.txt | read_complete | implemented_gap, partial | Added persisted question-glossary linkage and retrieval, derived glossary audio programs for topic and question repair flows, learner-aware personalized audio programming, bundle-route/review-loop sequencing, and relationship-review prompts; richer audio delivery still remains |
| idea18.txt | read_complete | implemented_gap, partial | Added persisted multi-phase diagnostic battery assembly plus phase submission, attempt-level phase item IDs, topic analytics, root-cause hypotheses, adaptive phase retargeting, confidence-distortion detection, a full completion sync that pushes diagnostic outputs into learner truth, coach profiles, blockers, plan rewrites, and Journey refresh, durable longitudinal comparison/cause-evolution summaries with diagnostic read APIs, and verified app-facing command/Tauri access for diagnostic results, longitudinal summaries, and cause-evolution reads; richer adaptive orchestration still remains |
...
| idea20.txt | read_complete | implemented_gap, partial | Added content-readiness gating, canonical next-action resolution, subject readiness engine, plan rewrite support, Journey route/runtime command exposure, today-mission generation exposure, persisted journey station evidence, diagnostic-driven plan/Journey resync, case-driven mission selection, evidence-gated Journey completion/retry rules, Journey-aware mission generation/completion feedback, and coach-state resolution that now reacts to active Journey review/performance/stall signals; deeper coach memory and mission adaptation still missing |
...
| idea30.txt | read_complete | implemented_gap, partial | Added availability/capacity substrate plus free-now recommendation and daily replanning orchestration that now reacts to repair success, repair failure, and memory-return pressure, and verified app-facing command/Tauri access for those scheduling outputs |
| idea31.txt | read_complete | implemented_gap, partial | Added publish jobs, quality reports, quality-gated publish transitions, source-to-publish orchestration, topic package gating, publish staging from the foundry coordinator, durable foundry jobs for publish/build follow-up, executable foundry automation for staging/quality/publish activation, source-aware note/formula/worked-example/question/contrast builders that now consume reviewed source candidates and approved evidence instead of only generic placeholders, and stronger auto-review/follow-up reconciliation so parse refreshes resolve obsolete review tasks and prefer quality follow-up over duplicate publish restaging; broader content-build automation still missing |
...
| idea33.txt | read_complete | implemented_gap, partial | Added contrast-profile ingestion, trap round/session persistence, Difference Drill, Similarity Trap, Know the Difference, Which Is Which, Unmask runtime flows, replay review, learner confusion state, and richer remediation/follow-up signals in trap review and game summaries; deeper orchestration is still missing |
| idea34.txt | read_complete | implemented_gap, partial | Added diagnostic phase submission, phase-aware topic analytics, durable root-cause hypotheses, adaptive next-phase retargeting, confidence-weighted distortion diagnosis, and persisted longitudinal/cause-evolution analytics with direct backend read methods; fuller adaptive orchestration is still missing |
...
| idea36.txt | read_complete | implemented_gap, partial | Extended upload intelligence with sidecar OCR/text recovery for scans and PDFs, optional native binary OCR/text fallback (`tesseract`/`pdftotext`) when available locally, page-aware recovery summaries, richer document-role classification (`report_card`, `corrected_script`, `teacher_handout`), mined dates/topics/question blocks/score signals/remarks/glossary terms, coach-action and student-model update signals, confirmation/alignment states, reason-coded question/answer alignment summaries, multi-page scan stitching, structured `docx`/`xlsx` sidecar extraction, native `docx`/`xlsx` package extraction, intake app-boundary commands, and richer normalized follow-through via reconstruction confidence bands, topic action summaries, coach-goal signals, and follow-up recommendations; deeper cross-engine consumption still remains outside the intake crate |
| idea37.txt | read_complete | implemented_gap, partial | Added learner-truth snapshot read model with topic, skill, memory, and diagnosis projections, plus topic-truth recompute/sync from memory decay and answer processing so active gap plans follow the latest learner state; broader cross-engine fabric still missing |
| idea38.txt | read_complete | implemented_gap, partial | Added source staging, acquisition jobs, learner evidence fabrics, publish/trust job substrate, topic package snapshots, subject foundry dashboards, a coordinator that links source review, topic health, and publish readiness, durable foundry job orchestration, executable foundry queue automation with run/run-next behavior, source-aware Foundry builders that carry reviewed curriculum/support signals through curriculum, knowledge, question, and contrast artifact creation, richer foundry/premium/reporting fabric summaries based on recent runtime evidence, and stronger reconciliation of auto-review tasks plus follow-up job chaining from review/acquisition into quality/publish flow; the broader Foundry/Content OS is still not fully complete |
