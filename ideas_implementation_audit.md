...
| idea31.txt | read_complete | implemented_gap, partial | Added publish jobs, quality reports, quality-gated publish transitions, source-to-publish orchestration, topic package gating, publish staging from the foundry coordinator, durable foundry jobs for publish/build follow-up, and executable foundry automation for staging, quality review, and publish activation; broader content-build automation still missing |
...
| idea35.txt | read_complete | implemented_gap, partial | Added a DTO-only runtime command boundary crate with shared AppState, unified CommandError, and a broad Tauri-registered app surface covering identity, coach, curriculum, sessions, attempts, foundry, diagnostics, questions, games, traps, library, memory, repair, readiness, premium, and mock-centre; remaining work is now feature-depth rather than boundary coverage |
...
| idea38.txt | read_complete | implemented_gap, partial | Added source staging, acquisition jobs, learner evidence fabrics, publish/trust job substrate, topic package snapshots, subject foundry dashboards, a coordinator that links source review, topic health, and publish readiness, durable foundry job orchestration, and executable foundry queue automation with run/run-next behavior; the broader Foundry/Content OS is still not fully complete |
